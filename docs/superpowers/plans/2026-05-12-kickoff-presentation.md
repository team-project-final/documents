# 킥오프 발표 문서 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** W1 첫날 킥오프에서 팀장이 팀원에게 프로젝트 전체를 안내하는 단일 마크다운 문서(`docs/project-management/KICKOFF.md`)를 작성한다.

**Architecture:** 9개 섹션 시간순 플로우형 단일 파일. 스펙의 §1~§9 설계를 그대로 반영하되, 실제 프로젝트 데이터(레포 URL, 팀원 실명, PRD 일정 등)를 기존 문서에서 가져와 채운다.

**Tech Stack:** Markdown. 참조 소스: DESIGN.md, README.md, PRD_W1~W5, onboarding 가이드, 부트스트랩 보고서.

**Spec reference:** [`docs/superpowers/specs/2026-05-12-kickoff-presentation-design.md`](../specs/2026-05-12-kickoff-presentation-design.md)
...
└── KICKOFF.md              # 신규 생성 — 킥오프 발표 문서 (9섹션, ~400줄)
```

---

## Task 1: KICKOFF.md 전체 작성

**Files:**
- Create: `docs/project-management/KICKOFF.md`

**참조할 기존 문서** (내용 추출용):
- `DESIGN.md` — 프로젝트 개요, 디자인 키워드
- `docs/project-management/README.md` — 문서 체계 §1~§9, 담당자 매핑표
- `docs/project-management/prd/PRD_W1.md` ~ `PRD_W5.md` — 주차별 목표/기간
- `docs/onboarding/00-common-day1.md` — Day 0 체크리스트 항목
- `docs/onboarding/README.md` — 온보딩 흐름

---

- [ ] **Step 1: 참조 문서에서 데이터 수집**

다음 파일들을 읽어 킥오프 문서에 넣을 데이터를 추출한다:

1. `DESIGN.md` 1~30줄 → 프로젝트 한 줄 정의, 대상 사용자, 핵심 차별점
2. `docs/project-management/README.md` §8 담당자 매핑표 → 팀 구성 테이블 데이터 확인
3. `docs/project-management/prd/PRD_W1.md` ~ `PRD_W5.md` 각 파일의 `## 1. 주차 개요` 테이블 → 기간, 목표 추출
4. `docs/onboarding/00-common-day1.md` → Day 0 체크리스트 항목 확인
5. `docs/onboarding/README.md` → Day 1 흐름 확인

---

- [ ] **Step 2: KICKOFF.md §1~§3 작성 (프로젝트 개요 + 팀 구성 + 기술 스택)**

`docs/project-management/KICKOFF.md`를 생성한다. 톤은 반말 캐주얼체.
...
- [ ] **Step 3: KICKOFF.md §4 작성 (GitHub 레포 + CI/CD)**
...
- [ ] **Step 5: KICKOFF.md §6 작성 (문서 체계 + 10단계 워크플로)**
...
- [ ] **Step 6: KICKOFF.md §7 작성 (Git 워크플로)**
...
- [ ] **Step 8: KICKOFF.md §9 작성 (Day 0 / Day 1 체크리스트 + 마무리)**
...
작성된 KICKOFF.md를 처음부터 끝까지 읽으며 다음을 확인:

1. 모든 팀원 이름 7명 누락 없는지 (김민구, 김해준, 한승완, 김현지, 박은서, 김나경, 조유지)
2. 모든 레포 URL 10개 링크 동작 확인 (형식: `https://github.com/team-project-final/{name}`)
3. 5주 로드맵 날짜가 PRD와 일치하는지
4. 반말 캐주얼체 일관성 (합니다/습니다 혼용 없는지)
5. 마크다운 렌더링 확인 (테이블, 코드블록, 체크박스)
...
```bash
git add docs/project-management/KICKOFF.md
git commit -m "docs(project-management): 킥오프 발표 문서 추가 (9섹션 시간순 플로우형)

W1 첫날 팀장→팀원 발표용. 프로젝트 개요 / 팀 배치(7명 실명) /
기술 스택 / 레포+CI/CD / 5주 로드맵 / 문서 체계 / Git 워크플로 /
소통 규칙 / Day 0·1 체크리스트.
...
| 스펙 섹션 | Task Step 매핑 | 커버 |
|-----------|---------------|:----:|
| §1 프로젝트 한눈에 보기 | Step 2 | ✅ |
| §2 팀 구성 + 담당 배치 | Step 2 | ✅ |
| §3 기술 스택 | Step 2 | ✅ |
| §4 GitHub 레포 + CI/CD | Step 3 | ✅ |
| §5 5주 로드맵 | Step 4 | ✅ |
| §6 문서 체계 + 10단계 워크플로 | Step 5 | ✅ |
| §7 Git 워크플로 | Step 6 | ✅ |
| §8 소통 규칙 | Step 7 | ✅ |
| §9 Day 0 / Day 1 | Step 8 | ✅ |
| 검증 + commit | Step 9~10 | ✅ |
...
TBD/TODO/"implement later"/"add appropriate" 패턴 없음. 모든 Step에 실제 마크다운 콘텐츠 포함.

### 3. 일관성 점검

- 팀원 이름 7명: §2 테이블, §9 Day 1 트랙 배정에서 동일 ✓
- 레포 이름 10개: §2, §4 테이블에서 동일 URL 패턴 ✓
- 주차 날짜: §5 테이블이 PRD_W1~W5 주차 개요와 일치 ✓
- 톤: 전체 반말 캐주얼체 ✓
