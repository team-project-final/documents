...
### 1.9 Controller + Test
- [x] POST /api/v1/ai/generate 엔드포인트 구현
...
- [x] embeddings 테이블 설계 (id, source_id, source_type, vector(1536), created_at)
- [x] pgvector 확장 활성화 (`CREATE EXTENSION vector`)
- [x] 인덱스 설계 (ivfflat 또는 hnsw)
- [x] Alembic 마이그레이션 작성
- [x] Duration(final) 갱신
...
### 1.7 Repository 구현
- [x] EmbeddingRepository 클래스 작성 (async SQLAlchemy)
- [x] pgvector 확장 + embeddings 테이블 Alembic 마이그레이션
...
### 1.9 Controller + Test
- [x] POST /api/v1/ai/embed 엔드포인트 구현
- [x] 배치 크기 검증 (최대 20건)
